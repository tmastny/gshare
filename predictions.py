...
def simulate_prediction(branch_patterns, predictor_type="1bit"):
    ...
    for i in range(100):  # Simulate 100 branches
        for addr in branch_patterns:
            ...
            # Make predictions
            concat_prediction = concat_pred.predict(concat_entry['addr'], concat_entry['history'])
            gshare_prediction = gshare_pred.predict(gshare_entry['addr'], gshare_entry['history'])

            # Update predictors
            concat_pred.update(concat_entry['addr'], concat_entry['history'], concat_entry['next_bit'])
            gshare_pred.update(gshare_entry['addr'], gshare_entry['history'], gshare_entry['next_bit'])
