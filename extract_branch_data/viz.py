...
def analyze_pattern_lengths(branch_history, min_length=4, max_length=16):
    ...
    plt.plot(
        [k for k in length_stats.keys()],
        [
            v["top_3_counts"][1] if len(v["top_3_counts"]) > 1 else 0
            for v in length_stats.values()
        ],
        "o-",
        label="2nd most frequent",
    )
    plt.plot(
        [k for k in length_stats.keys()],
        [
            v["top_3_counts"][2] if len(v["top_3_counts"]) > 2 else 0
            for v in length_stats.values()
        ],
        "o-",
        label="3rd most frequent",
    )
...
def calculate_gini(values):
    ...
    mean = sum(values) / n
    abs_diff_sum = sum(
        abs(xi - xj) for i, xi in enumerate(values) for j, xj in enumerate(values)
    )
...
def calculate_metrics(branch_history, min_length=4, max_length=10):
    ...
    for length, metrics in results.items():
        print(
            f"{length:6d} | {metrics['entropy']:7.3f} | {metrics['gini']:5.3f} | {metrics['unique_patterns']:>15d}"
        )
...
def plot_pattern_frequency_heatmap(branch_history, min_length=4, max_length=10):
    ...
    for length in range(min_length, max_length + 1):
        ...
        frequencies = sorted(
            [count / total for count in pattern_counts.values()], reverse=True
        )
...
def main():
    ...
    parser.add_argument(
        "--window", type=int, default=32, help="Window size for heatmap (default: 32)"
    )
    parser.add_argument(
        "--pattern",
        type=int,
